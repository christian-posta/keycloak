...
Pseudonym (require=pseudonym): Signed request proves possession of an included public key, providing consistent identity without verification¶
Identified (require=identity): Agent identity verified via JWKS or agent token¶
Authorized (require=auth-token): Full authorization with auth token¶
4.3.2. Pending States
These are returned as 202 responses during authorization, indicating what must happen before the request can complete:¶

Interaction (require=interaction): The agent must direct the user to the interaction endpoint with the provided code¶
Approval (require=approval): The auth server is obtaining approval directly from a user or policy evaluation — the agent polls until resolved¶
4.4. Scopes
...
16.1. HTTP Signature Verification
Extract the Signature, Signature-Input, and Signature-Key headers. If any are missing, return invalid_signature.¶
Verify that the Signature-Input covers the required components: @method, @authority, @path, and signature-key. If the resource requires additional components via additional_signature_components, verify those are covered as well.¶
Verify the created parameter is present and within 60 seconds of the current time. Reject if outside this window.¶
...
Verify the HTTP Message Signature ([RFC9421]) using the obtained public key.¶
16.2. JWT Verification
...
Decode the JWT header. Verify typ matches the expected token type (agent+jwt or auth+jwt).¶
Verify the JWT signature using the issuer's public key, obtained by fetching the issuer's JWKS from jwks_uri in the issuer's metadata and selecting the key matching the JWT's kid header parameter. Keys are JSON Web Keys as defined in [RFC7517].¶
Verify the exp claim is in the future. Verify the iat claim is not in the future.¶
Key binding: Verify that the public key in the JWT's cnf claim ([RFC7800]) matches the key used to sign the HTTP request. This binds the JWT to the request signer.¶
16.2.1. Agent Token Verification
When verifying an agent token (typ: agent+jwt):¶

Perform JWT Verification above using the agent server's JWKS (fetched from jwks_uri in /.well-known/aauth-agent.json at the iss URL).¶
Verify iss is a valid HTTPS URL conforming to the Server Identifier requirements.¶
Verify the cnf.jwk matches the key used to sign the HTTP request.¶
If aud is present, verify that the server's own identifier is listed in the aud claim. If the server's identifier is not listed, reject the token.¶
16.2.2. Auth Token Verification
When verifying an auth token (typ: auth+jwt):¶

Perform JWT Verification above using the auth server's JWKS (fetched from jwks_uri in /.well-known/aauth-issuer.json at the iss URL).¶
Verify iss is a valid HTTPS URL conforming to the Server Identifier requirements.¶
Verify aud matches the resource's own identifier (or the agent's identifier for self-access tokens).¶
Verify agent matches the agent identifier from the request's signing context.¶
Verify cnf.jwk matches the key used to sign the HTTP request.¶
Verify that at least one of sub or scope is present.¶
16.2.3. Resource Token Verification
When an auth server receives a resource token (typ: resource+jwt) in a token request:¶

Perform JWT Verification above using the resource's JWKS (fetched from jwks_uri in /.well-known/aauth-resource.json at the iss URL).¶
Verify aud matches the auth server's own identifier.¶
Verify agent matches the requesting agent's identifier.¶
Verify agent_jkt ([RFC7638]) matches the JWK Thumbprint of the key used to sign the HTTP request.¶
Verify exp is in the future.¶
Verify jti has not been seen before (replay detection). The auth server SHOULD maintain a cache of seen jti values for the token's lifetime.¶
If txn is present, carry it forward into the issued auth token.¶
17. Response Verification
...
Verify the auth token JWT per the JWT Verification steps above, using the auth server's JWKS.¶
Verify iss matches the auth server the agent sent the token request to.¶
Verify aud matches the resource the agent intends to access (or the agent's own identifier for self-access tokens).¶
...
Extract the resource-token and auth-server parameters from the AAuth header.¶
Decode the resource token JWT header and verify typ is resource+jwt.¶
Verify the resource token JWT signature using the resource's JWKS (fetched from jwks_uri in /.well-known/aauth-resource.json).¶
Verify iss matches the resource the agent sent the request to.¶
Verify agent matches the agent's own identifier.¶
Verify agent_jkt matches the JWK Thumbprint of the agent's signing key.¶
Verify exp is in the future.¶
...
Request Integrity: The signature covers HTTP method, target URI, and headers¶
Replay Protection: The created timestamp limits signature validity¶
Key Binding: Signatures are bound to specific keys via Signature-Key¶
20.3. Token Security
...
Hardt, D. and T. Meunier, "HTTP Signature-Key Header", Work in Progress, Internet-Draft, draft-hardt-httpbis-signature-key-01, 8 January 2026, <https://datatracker.ietf.org/doc/html/draft-hardt-httpbis-signature-key-01>.
[RFC3986]
...
[RFC8941]
Nottingham, M. and P. Kamp, "Structured Field Values for HTTP", RFC 8941, DOI 10.17487/RFC8941, February 2021, <https://www.rfc-editor.org/info/rfc8941>.
[RFC9110]
Fielding, R., Ed., Nottingham, M., Ed., and J. Reschke, Ed., "HTTP Semantics", STD 97, RFC 9110, DOI 10.17487/RFC9110, June 2022, <https://www.rfc-editor.org/info/rfc9110>.
[RFC9421]
Backman, A., Ed., Richer, J., Ed., and M. Sporny, "HTTP Message Signatures", RFC 9421, DOI 10.17487/RFC9421, February 2024, <https://www.rfc-editor.org/info/rfc9421>.
22.2. Informative References
[OpenID.Core]
